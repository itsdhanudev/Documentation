...
| --- | --- |
| UIBloom([UIBloom]) | Executes `UIBloom` and returns after the underlying engine call finishes. |
| UIBloom([Engine]) | Executes `UIBloom` and returns after the underlying engine call finishes. |
| UIBloom() | Executes `UIBloom` and returns after the underlying engine call finishes. |

## Attributes (1)

| Name | Type | Description |
| --- | --- | --- |
| `instance` | [UIBloom] | Represents the `instance` value stored on this instance. Read or assign to update the property. |
