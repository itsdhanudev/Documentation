...
| --- | --- |
| VHSFilter([VHSFilter]) | Executes `VHSFilter` and returns after the underlying engine call finishes. |
| VHSFilter([Engine]) | Executes `VHSFilter` and returns after the underlying engine call finishes. |
| VHSFilter() | Executes `VHSFilter` and returns after the underlying engine call finishes. |

## Attributes (1)

| Name | Type | Description |
| --- | --- | --- |
| `instance` | [VHSFilter] | Represents the `instance` value stored on this instance. Read or assign to update the property. |
