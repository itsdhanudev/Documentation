# Vector3.genericInterface()

Static utility on `Vector3`.

## Description

- Executes `genericInterface` and returns after the underlying engine call finishes. |
    ...
    <script>
      const sketch = (p) => {
        ...
        p.draw = () => {
          ...
          p.fill('#dbeafe');
          p.noStroke();
          p.text('Vector3 topic: vector3.genericinterface', 16, 24);
...
<iframe
  title="p5-vector3-genericinterface"
