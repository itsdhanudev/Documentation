# Vector3.inspectorController()

Static utility on `Vector3`.

## Description

- Executes `inspectorController` and returns after the underlying engine call finishes. |
...
- `ClassInspectorEntry`: returned by overloads of this method.
    ...
    <script>
      const sketch = (p) => {
        ...
        p.draw = () => {
          ...
          p.fill('#dbeafe');
          p.noStroke();
          p.text('Vector3 topic: vector3.inspectorcontroller', 16, 24);
...
<iframe
  title="p5-vector3-inspectorcontroller"
