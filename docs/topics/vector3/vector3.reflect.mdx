...
- Reflects direction vector off a plane defined by the normal. |
- Reflects direction vector off a plane defined by the normal and stores the result in the specified vector. |
    ...
    <script>
      const sketch = (p) => {
        ...
        p.draw = () => {
          ...
          p.fill('#dbeafe');
          p.noStroke();
          p.text('Vector3 topic: vector3.reflect', 16, 24);
