# Vector3.serializeToString()

Instance method on `Vector3`.

## Description

- Returns a String representation of this Vector3 in the format '[X;Y;Z]'. |
    ...
    <script>
      const sketch = (p) => {
        ...
        p.draw = () => {
          ...
          p.fill('#dbeafe');
          p.noStroke();
          p.text('Vector3 topic: vector3.serializetostring', 16, 24);
...
<iframe
  title="p5-vector3-serializetostring"
