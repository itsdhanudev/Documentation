...
- Returns the squared length of this Vector3. Faster than length() as it doesn't use a square root; useful for comparing distances. |
    ...
    <script>
      const sketch = (p) => {
        ...
        p.draw = () => {
          ...
          p.fill('#dbeafe');
          p.noStroke();
          p.text('Vector3 topic: vector3.sqrlength', 16, 24);
