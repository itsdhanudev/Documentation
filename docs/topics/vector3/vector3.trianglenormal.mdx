...
- Calculates the normal of the triangle defined by the three specified vertices. |
- Calculates the normal of the triangle defined by the three specified vertices and stores the result in the specified vector. |

## Method Signature

```java
triangleNormal(Vector3, Vector3, Vector3)
triangleNormal(Vector3, Vector3, Vector3, Vector3)
```
    ...
    <script>
      const sketch = (p) => {
        ...
        p.draw = () => {
          ...
          p.fill('#dbeafe');
          p.noStroke();
          p.text('Vector3 topic: vector3.trianglenormal', 16, 24);
...
<iframe
  title="p5-vector3-trianglenormal"
