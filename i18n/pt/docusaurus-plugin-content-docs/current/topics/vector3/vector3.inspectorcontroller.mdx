# Vetor3.inspectorController()

Static utility on `Vector3`.

## Descrição

- Executes `inspectorController` and Retorna after the underlying engine call finishes. |
    ...
    <script>
      const sketch = (p) => {
        ...
        p.draw = () => {
          ...
          p.fill('#dbeafe');
          p.noStroke();
          p.text('Tópico Vector3: vector3.inspectorcontroller', 16, 24);
