...
- Smoothly moves o atual vector towards the target vector with smoothing control, maximum speed, in the defined interval. |
- Smoothly moves o atual vector towards the target vector with smoothing control, maximum speed, in the defined interval and stores the result in the specified vector. |
    ...
    <script>
      const sketch = (p) => {
        ...
        p.draw = () => {
          ...
          p.fill('#dbeafe');
          p.noStroke();
          p.text('Tópico Vector3: vector3.smoothdamp', 16, 24);
