...
- Adds the specified Vector3 to this Vector3 and Retorna Um novo Vector3. |
- Adds the specified X, Y and Z values to this Vector3 and Retorna Um novo Vector3. |
- Adds the specified value to this Vector3 and Retorna Um novo Vector3. |
- Retorna the sum of this Vector3 and the specified object. |
    ...
    <script>
      const sketch = (p) => {
        ...
        p.draw = () => {
          ...
          p.fill('#dbeafe');
          p.noStroke();
          p.text('Tópico Vector3: vector3.sum', 16, 24);
